...
public class OwnerControllerUserTest {
    @Autowired
    ...
    @Test
    void findAll_WhenNotAdmin_ShouldReturnAllOwners() throws Exception {
    ...
    @Test
    void findById_WhenNotAdmin_ShouldReturnForbidden() throws Exception {
    ...
    @Test
    void update_WhenNotAdmin_ShouldReturnForbidden() throws Exception {
    ...
    @Test
    void create_WhenNotAdmin_ShouldCreateOwner() throws Exception {
    ...
    @Test
    @WithAnonymousUser
    void create_WhenAnonymousUser_ShouldCreateOwner() throws Exception {
        OwnerDto ownerDto = getOwners().getFirst();
        String requestBody = """
                {
                "name": "masha",
    ...
    @Test
    void getOwnersWithFilters_WhenNotAdmin_ShouldReturnOwners() throws Exception {
        OwnerDto ownerDto = getOwners().getFirst();
    ...
    @Test
    void deleteById_WhenNotAdmin_ShouldReturnForbidden() throws Exception {
        long ownerId = 1L;
        Mockito.doNothing().when(ownerService).deleteById(ownerId);
        mockMvc.perform(delete("/api/owners/{id}", ownerId).with(user(userDetails)))
                .andExpect(status().isForbidden());
        Mockito.verify(ownerService, times(0)).deleteById(ownerId);
    }

    @Test
    void deleteByEntity_WhenNotAdmin_ShouldReturnForbidden() throws Exception {
        OwnerDto ownerDto = getOwners().getFirst();
        String requestBody = """
                {
                "id": 1,
        ...
        mockMvc.perform(delete("/api/owners")
                        ...
                        .with(user(userDetails)))
                .andExpect(status().isForbidden());
        Mockito.verify(ownerService, times(0)).deleteByEntity(
                argThat(dto -> dto.getId().equals(1L) && dto.getName().equals("masha")
                ));
    }

    @Test
    void deleteAll_WhenNotAdmin_ShouldReturnForbidden() throws Exception {
